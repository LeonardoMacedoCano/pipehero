...
const missedKeys = new Set([noteRenderKey(missedNote.fret, missedNote.time)]);
let missDrawSucceeded = true;
try {
  drawFrame(ctx, notes, missedAt + 0.12, RENDER_CONFIG, new Map(), new Set(), missedKeys);
  save("07-missed-note-desaturated.png");
} catch (err) {
